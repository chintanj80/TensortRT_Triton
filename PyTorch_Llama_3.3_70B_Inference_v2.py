    ...
    def generate_response(
        ...
        with torch.no_grad():
            if stream and measure_time:
                ...
                def first_token_callback(idx, token):
                    nonlocal first_token_time
                    if first_token_time is None:
                        first_token_time = time.time()
                    return False
                
                streamer.on_finalized_text = first_token_callback
                ...
                if measure_time and first_token_time is not None:
                    metrics["time_to_first_token"] = first_token_time - gen_start_time
            else:
                output = self.model.generate(
                    **inputs,
                    max_new_tokens=max_new_tokens,
...
# Streamer class for measuring time to first token
class TextStreamer:
    ...
    def put(self, token_idx):
        if self.on_finalized_text:
            self.on_finalized_text(token_idx, token_idx)
