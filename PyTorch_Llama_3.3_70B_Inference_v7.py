...
class Llama33Inference:
    ...
    def run_benchmark(
        ...
        **kwargs
        ...
        benchmark_results = {
            "device": self.device,
            "device_info": torch.cuda.get_device_name(0) if torch.cuda.is_available() and self.device == "cuda" else "CPU",
            "model": self.model_id,
            "num_runs": num_runs,
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Benchmark Llama 3.3 70B model")
    ...
    parser.add_argument("--stream", action="store_true", 
                        help="Enable streaming to measure time to first token")
    ...
    try:
        ...
        llm = Llama33Inference(device=args.device)
        ...
        try:
            ...
            print("Checking if model is loaded correctly...")
            if hasattr(llm, 'model') and llm.model is not None:
                print("Model is loaded. Proceeding with caution.")
            else:
                print("Model failed to load. Please check your environment.")
                import sys
                sys.exit(1)
